...
import com.autozone.database.dao.PrestamoDAO;
...
public class Principal {
	public static void main(String[] args){		
		 ...
		 *Principalmente sirve para leer las entradas que escribe el usuario,
		 *pero tambien puede tener validaciones y ciclos para una experiencia mas comoda.
		 */
	...
	private static void actualizarLib() {
		...
		try(Scanner scanner = new Scanner(System.in); Scanner scanner1 = new Scanner(System.in)){
			//Se declaran las variables a usar y se guardan las respuestas del usuario en estas.
			String titulo, autor; int isbn;
			System.out.println("Cual es el ISBN del libro que quiere actualizar?");
			isbn = scanner1.nextInt();
			System.out.println("Nuevo titulo del libro:");
			titulo = scanner.nextLine();
			System.out.println("Nuevo autor del libro:");
			autor = scanner.nextLine();
			/**Luego se crea una lista donde se almacena informacion de un libro en especifico
			 * Esto se hizo para poder tomar la disponibilidad de un libro en especifico
			 * y poder usar este status para actualizar un registro en la base de datos.
			 * */
			List<Libros> libros = libroDAO.obtenerEstatusDeLibro(isbn);
			for (Libros libro : libros) {
				String disp = libro.getDisponibilidad().toString();
				Libros libro1 = new Libros(titulo, autor, disp);
				libroDAO.actualizarLibro(isbn, libro1);
			}
	...
	private static void buscarPorISBN() {
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			//Aqui se leen los datos de una lista y se les da un poco de formato para presentarse al usuario
	...
	public static void agregLib() {
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			Libros libro = new Libros(titulo, autor, LibroStatus.DISPONIBLE.toString());
			//Luego, se manda al metodo insertar libro para poder entregarse a la base de datos.
	...
	private static void buscarPorNombre() {
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			for (Miembros miembro : miembros) {
				System.out.println("| " + miembro.getId()+" | "+miembro.getNombre()+" | "+miembro.getTelefono()+" |");
			}
			menuRecursivo();
		} catch(Exception e) {
			e.printStackTrace();
	...
	private static void historialDePrestamosDeMiembro() {
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			for (Prestamos prestamo : prestamos) {
				System.out.println("| " + prestamo.getFolio()+" | "+prestamo.getIsbn()+" | "+prestamo.getId()+" | "+prestamo.getFecha()+" | "+prestamo.getLibro_operacion()+" |");
			}
			menuRecursivo();
		} catch(Exception e) {
			e.printStackTrace();
	...
	public static void operacionPrestamo() {		
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			int isbn = scannerAgregar.nextInt();
			List<Libros> libros = libroDAO.obtenerEstatusDeLibro(isbn);
			//Aqui se toma un valor de la lista de libros para validar el curso de accion a tomar.
			switch (libros.get(0).getDisponibilidad()) {
			case "DISPONIBLE": {
				System.out.println("ID de miembro:");
				int id = scannerAgregar.nextInt();
	...
	private static void estatusLibro() {
		...
		try(Scanner scannerAgregar = new Scanner(System.in)){
			...
			for (Libros libro : libros) {
				System.out.println("| " + libro.getIsbn()+" | "+libro.getTitulo()+" | "+libro.getDisponibilidad()+" |");
			}
			menuRecursivo();
		} catch(Exception e) {
			e.printStackTrace();
